...
function SurveyDetails() {
	...
	async function LoadDataTrial() {
		if (contract !== null) {
			setstatus("loading...");
			setTRIAL_DATA({});
			let trial_element = await ReadContract(api, signerAddress, "_trialMap", [parseInt(location.state.trialID)]);
			var newTrial = {
	...
	async function LoadSurveyData() {
		if (contract !== null) {
			setstatus("loading...");
			let survey_element = await ReadContract(api, signerAddress, "_surveyMap", [parseInt(params.id)]);
			var new_survey = {
	...
	async function LoadDataCategories() {
		try {
			...
			const totalSurveyCategory = await ReadContract(api,signerAddress, "_SurveyCategoryIds")    
		
				for (let i = 0; i < Number(totalSurveyCategory); i++) {
					const element = await ReadContract(api, signerAddress, "_categoryMap", [parseInt(i)]);
					setdataCategory((prevState) => [
						...prevState,
						{
							value: element.name,
